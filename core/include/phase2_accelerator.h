#ifndef QALLOW_PHASE2_ACCELERATOR_H
#define QALLOW_PHASE2_ACCELERATOR_H
 ...
 */
typedef struct phase2_accel_config_s {
    size_t thread_count;
...
} phase2_accel_config_t;

/**
 * Parse CLI arguments and launch the accelerator. Mirrors the behaviour of the standalone binary.
 */
int qallow_phase2_main(int argc, char** argv);

/**
 * Launch the accelerator with a programmatic configuration.
 */
int qallow_phase2_accel_start(const phase2_accel_config_t* config);

#ifdef __cplusplus
}
#endif

#endif /* QALLOW_PHASE2_ACCELERATOR_H */
