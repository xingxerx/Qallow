...
static int run_phase(const char* phase, int (*runner)(int, char**), int argc, char** argv) {
    ...
    QALLOW_PROFILE_SCOPE("throughput_benchmark_phase");
    int rc = runner(argc, argv);
    double elapsed = wall_ms() - start;

    qallow_log_info("benchmark.phase", "phase", phase, "elapsed_ms", elapsed);
    return rc;
}
...
int main(int argc, char** argv) {
    ...
    if (run_phase("phase12", qallow_phase12_runner, 4, (char**)phase12_args) != 0) {
        qallow_log_error("benchmark.phase12.failed", "ticks", 200);
        return EXIT_FAILURE;
    }
    ...
    if (run_phase("phase13", qallow_phase13_runner, 5, (char**)phase13_args) != 0) {
        qallow_log_error("benchmark.phase13.failed", "nodes", 16, "ticks", 400);
        return EXIT_FAILURE;
    }

    qallow_log_info("benchmark.complete", "phases", 2);
