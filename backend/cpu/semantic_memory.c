...
static MDB_env* env_nodes = NULL;
static MDB_env* env_edges = NULL;
...
int smg_init(const char* db_path) {
    ...
    // Initialize LMDB for nodes
    if (mdb_env_create(&env_nodes) == 0) {
        char node_path[512];
        snprintf(node_path, sizeof(node_path), "%s/nodes", smg_db_path);
        mdb_env_open(env_nodes, node_path, 0, 0664);
    }

    // Initialize LMDB for edges
    if (mdb_env_create(&env_edges) == 0) {
        char edge_path[512];
        snprintf(edge_path, sizeof(edge_path), "%s/edges", smg_db_path);
        mdb_env_open(env_edges, edge_path, 0, 0664);
    }
