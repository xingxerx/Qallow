...
int run_phase1_elasticity(const char* audit_tag,
    ...
    if (qallow_phase_resolve_log_path("phase1", requested_log_path, csv_path, sizeof(csv_path)) != 0) {
        fprintf(stderr, "[PHASE1] Failed to prepare log path (requested=%s)\n",
    ...
    if (!f) {
        fprintf(stderr, "[PHASE1] Unable to open log file: %s\n", csv_path);
        return 1;
    }
    fprintf(f,
    ...
    printf("[PHASE1] Elastic run complete: ticks=%d eps=%.6f\n", ticks, eps);
    printf("[PHASE1] Coherence≈%.6f EntropyΔ≈%.6f Deco≈%.6f\n", coherence, entropy, deco);

    if (qallow_phase_update_latest_symlink("phase1", csv_path) != 0) {
        fprintf(stderr, "[PHASE1] Warning: failed to refresh latest symlink for %s\n", csv_path);
    }
    ...
    if (qallow_phase_write_summary("phase1", tag, csv_path, metrics_json) != 0) {
        fprintf(stderr, "[PHASE1] Warning: failed to write phase_summary.json\n");
    }
    ...
    learn_event_t ev = {
        .phase = "phase1",
        .module = "elasticity",
        .objective_id = "phase1.elasticity",
