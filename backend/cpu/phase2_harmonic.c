...
#define QALLOW_PHASE2_MAX_POCKETS 32
...
int run_phase2_harmonic(const char* audit_tag,
    ...
    if (ticks <= 0) {
        fprintf(stderr, "[PHASE2] Invalid tick count: %d\n", ticks);
        return 1;
    }

    if (pockets < 2) {
        fprintf(stderr, "[PHASE2] Pocket count too low, promoting to 2\n");
        pockets = 2;
    }

    if (pockets > QALLOW_PHASE2_MAX_POCKETS) {
        fprintf(stderr, "[PHASE2] Pocket count capped to %d\n", QALLOW_PHASE2_MAX_POCKETS);
        pockets = QALLOW_PHASE2_MAX_POCKETS;
    }

    char csv_path[PATH_MAX];
    if (qallow_phase_resolve_log_path("phase2", requested_log_path, csv_path, sizeof(csv_path)) != 0) {
        fprintf(stderr, "[PHASE2] Failed to prepare log path (requested=%s)\n",
    ...
    if (!log) {
        fprintf(stderr, "[PHASE2] Failed to open log file: %s\n", csv_path);
        return 2;
    }
    fprintf(log,
    ...
    float phase_curr[QALLOW_PHASE2_MAX_POCKETS];
    float phase_prev[QALLOW_PHASE2_MAX_POCKETS];
    float coh_curr[QALLOW_PHASE2_MAX_POCKETS];
    float coh_prev[QALLOW_PHASE2_MAX_POCKETS];
    ...
    memcpy(phase_prev, phase_curr, sizeof(float) * QALLOW_PHASE2_MAX_POCKETS);
    memcpy(coh_prev, coh_curr, sizeof(float) * QALLOW_PHASE2_MAX_POCKETS);
    ...
    printf("[PHASE2] Harmonic propagation complete: pockets=%d ticks=%d k=%.6f\n",
           pockets, ticks, coupling);
    printf("[PHASE2] avg_coherence: %.6f → %.6f\n", start_coherence, final_coherence);
    printf("[PHASE2] phase_drift  : %.6f → %.6f\n", start_drift, final_drift);

    if (qallow_phase_update_latest_symlink("phase2", csv_path) != 0) {
        fprintf(stderr, "[PHASE2] Warning: failed to refresh latest symlink for %s\n", csv_path);
    }
    ...
    if (qallow_phase_write_summary("phase2", tag, csv_path, metrics_json) != 0) {
        fprintf(stderr, "[PHASE2] Warning: failed to write phase_summary.json\n");
    }
