"""
ALG Test Module
Runs internal tests on small Hamiltonian (8-node ring)
"""
...
def run_internal_test():
    ...
    print("[ALG TEST] Running 8-node ring test...")
    ...
    try:
        ...
        print(f"[ALG TEST] ✗ Test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def check_results_file():
    ...
    if not os.path.exists(results_path):
        print(f"[ALG TEST] No results file: {results_path}")
    ...
    try:
        ...
        print(f"[ALG TEST] Results file: {results_path}")
        print(f"[ALG TEST]   Energy: {data.get('energy', 'N/A'):.6f}")
        print(f"[ALG TEST]   Alpha_eff: {data.get('alpha_eff', 'N/A'):.6f}")
        print(f"[ALG TEST]   Iterations: {data.get('iterations', 'N/A')}")
...
def exec(args=None):
    ...
    print("\n" + "="*70)
    print("ALG TEST - Internal Validation")
    print("="*70 + "\n")
    ...
    if quick_mode:
        ...
        print("[ALG TEST] Full mode: running internal test\n")
        success = run_internal_test()
